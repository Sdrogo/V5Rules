...
class DisciplineViewModelTest {
    ...
    fun `initialization should fetch disciplines and update state to Success`() = runTest {
        val disciplines = listOf(Discipline(title = "Animalism"), Discipline(title = "Auspex"))
        every { mainRepository.loadDisciplines(any()) } returns disciplines

        val viewModel = DisciplineViewModel(mainRepository, testDispatcher)
        advanceUntilIdle()
        ...
        viewModel.disciplineUiState.test {
            ...
            assertEquals("Auspex", state.disciplines[1].title)
        }
    }
    ...
    @Test
    fun `initialization should sort disciplines by title`() = runTest {
        val disciplines = listOf(Discipline(title = "Celerity"), Discipline(title = "Animalism"))
        every { mainRepository.loadDisciplines(any()) } returns disciplines
        ...
        viewModel.disciplineUiState.test {
            val state = awaitItem()
            assertTrue(state is DisciplineUiState.Success)
            val successState = state as DisciplineUiState.Success
            assertEquals("Animalism", successState.disciplines[0].title)
            assertEquals("Celerity", successState.disciplines[1].title)
        }
    }
}
